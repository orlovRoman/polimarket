...
class ScoutAgent:
    ...
    def estimate_market(self, context: 'MarketContext') -> Optional[Signal]:
        ...
        description_block = market.description if market.description and len(market.description) > 20 else "⚠️ Описание отсутствует — оракул-риск не определён, требуется ручная проверка на сайте."
        prompt = f"""
