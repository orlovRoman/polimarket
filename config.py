...
CEREBRAS_API_KEY_SECONDARY = os.getenv("CEREBRAS_API_KEY_SECONDARY", "")
CEREBRAS_API_KEY_THIRD = os.getenv("CEREBRAS_API_KEY_THIRD", "")
...
def setup_logger(name="NexusPolyBot"):
    ...
    strategy_log = next(
        (v for k, v in STRATEGY_LOG_MAP.items() if k in name), None
    )
    if strategy_log:
        file_handler = RotatingFileHandler(LOGS_DIR / strategy_log, maxBytes=5*1024*1024, backupCount=3, encoding="utf-8")
    else:
