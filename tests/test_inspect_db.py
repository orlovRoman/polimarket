...
def test_inspect_active_penny_stocks():
    print(f"\nDB_PATH: {config.DB_PATH}")
    conn = sqlite3.connect(str(config.DB_PATH))
    conn.row_factory = sqlite3.Row
    cursor = conn.cursor()
    
    # Посмотрим схему таблицы penny_stocks_monitoring
    ...
    # Посмотрим активные penny stocks с NULL в predicted_outcome
    cursor.execute("""
    ...
    # Проверим, есть ли вообще какие-то записи с null ценами
    cursor.execute("""
