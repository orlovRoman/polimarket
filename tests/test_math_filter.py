...
def test_monotonicity_violation():
    ...
    assert result.has_arbitrage is False
    assert "⚠️ Трейд недоступен" in result.trade_instruction
...
def test_logical_implication_trade_contains_only_buy():
    """Трейд-инструкция для logical_implication должна содержать только BUY"""
    from core.models import Market
    from datetime import datetime, timedelta, timezone
