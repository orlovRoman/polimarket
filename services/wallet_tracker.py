...
from agents.shared.python.db import (
    get_connection, save_trader_transaction, update_wallet_stats, update_wallet_pvalue
)
...
def recalculate_win_rates() -> int:
    ...
    for row in rows:
        ...
        p_val = binomial_pvalue(total, wins)
        is_insider = (total >= 15 and p_val < 0.05)
        # Записываем расширенную статистику p-value в БД (BUG-WT-02)
        update_wallet_pvalue(row["wallet_address"], total, wins, p_val, is_insider)
